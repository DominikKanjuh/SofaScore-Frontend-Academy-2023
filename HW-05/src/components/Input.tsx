...
export default function Input() {
  ...
  const [circles, setCircles] = useState<Circle[]>(() => {
    const localData = localStorage.getItem("circles");
  ...
  const onClick = (e: React.SyntheticEvent) => {
    ...
    setCircles([...circles, getRandomCircleParameters(currentRadius)]); //spremanje u listu krugova

    setCurrentRadius(DEFAULT_SELECTED_RADIUS);
    //vracanje na default radius
  ...
  return (
    <div style={{ margin: "0 auto" }}>
      <form>
        <label>
          Radius of next circle: {currentRadius}&nbsp;
          <input
      ...
      <svg
        ...
        style={{
          width: VIEW_BOX_DIMENSION,
          height: VIEW_BOX_DIMENSION,
          border: "1px solid black",
        }}
      >
        {/* In this specific case, key is also the id of the circle since we don't delete any */}
        {circles.map((c, index) => (
          <circle key={index} cx={c.x} cy={c.y} r={c.r} fill={c.color} />
        ))}
      </svg>
    </div>
  );
